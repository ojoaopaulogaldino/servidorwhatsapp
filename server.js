...
const upload = multer({ storage, limits: { fileSize: 15 * 1024 * 1024 } }); // Limite de 15MB
...
// Gerenciamento de conexão com o WhatsApp
let sock = null;
let isConnected = false;
...
async function connectToWhatsApp() {
  ...
  try {
    ...
    sock = makeWASocket({
      printQRInTerminal: true,
      auth: state,
      version, // Usa a versão mais recente do Baileys
      browser: ['WhatsAuto', 'Chrome', '120.0.0']
    });
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (update.connection === 'close') {
        ...
        isConnected = false;
        io.emit('connection-status', false);
        io.emit('log', '❌ Conexão perdida. Tentando reconectar...');
        await handleReconnection(update.lastDisconnect);
      }
    });
...
async function handleReconnection(lastDisconnect) {
  const shouldReconnect = lastDisconnect?.error?.output?.statusCode !== 401; // 401 = autenticação inválida
  if (!shouldReconnect) {
    console.log('[WHATSAPP] Conexão encerrada. Reinicie o servidor.');
    return;
  }

  const baseDelay = Math.min(5000 * (reconnectAttempts + 1), 30000); // Backoff exponencial
  await delay(baseDelay);
  reconnectAttempts++;
  console.log(`[WHATSAPP] Tentativa de reconexão #${reconnectAttempts}`);
...
async function sendMessageWithMedia(recipient, content, mediaPath) {
  if (mediaPath) {
    try {
      ...
      console.log('[MENSAGEM] Mídia enviada com sucesso');
    } catch (error) {
      console.error('[ERRO] Falha ao enviar mídia:', error);
      throw error;
    }
  } else {
    await sock.sendMessage(recipient, { text: content });
